...
class IndieMuse(nn.Module):
    def __init__(self, hidden_sz=16, seq_len=32, note_range = 49):
        super(IndieMuse,self).__init__()
        """
        ----------------------------------------------------------------------
    ...
    def train(self, notes, lr_rate = 1e-3, epochs = 100, batch_sz = 128):
        ...
        ------------------------------------------------------------------------
        """
        #Establish seeds for generation.

        self.seeds = cur_notes.seeds 
    ...
    def create_music(self, seed_notes=False, midi_folder = "generated_samples",
    thresh=0.25, num_of_clips = 5, save_seed = False, random = True, use_model_seed=True):
        """
        ----------------------------------------------------------------------
        This uses IndieMuse's note prediction to create music. Given a
        ...
        --thresh--
        The threshold for deciding how likely a note has to be so we keep
        itself.
        ...
        --random--
        A bool which checks if you want to just generated random samples, as
        opposed to ordered.

        --use_model_seed-
        A bool which if True, uses the seeds established during training.
        Otherwise, seed_notes better true!
        ######################################################################
        ----------------------------------------------------------------------
        """
        for i in range(num_of_clips):
            idx = np.random.randint(0,self.seeds.shape[0])
            idx = idx if random else i
            seed = self.seeds[idx].unsqueeze(0)
