...
class Notes(Dataset):
    def __init__(self, path, processor=processor, MIDI = False , seq_len = 64,
                ...
                save_path = 'prepared_data/data', min_note = 36, max_note = 84):
        """
        ----------------------------------------------------------------------
        ...
        --processor--
        Checks whether to use a gpu or cpu. See top of the file for the
        definition.

        --MIDI--
        A bool which checks whether the songs are in MIDI format or if they
        ...
        --keys--
        A list of valid keys. If False, it'll default to all keys except for
        C major.

        --subdiv--
        The number of steps per subdivision. See midi_to_roll's
        description for more information.

        --save_path--
        A string denoting the folder to save the block of songs as torch
        tensor for later usage.
        ...
        if not MIDI:
            ...
            print('Preparing the data...') # Otherwise gather MIDIs and build dataset
            if os.path.exists(path):
                ...
                for song in songs:
                    try:
                        ...
                        midi_data.append(tensor[:, min_note:max_note+1])
                    except:
                        errors += 1
            else:
                raise Exception('Invalid Path! Maybe a typo?')
            print("Number of errors: ", errors)
            midi_data = torch.cat(midi_data,0).to(processor)
        ...
        N = self.cur_notes.shape[0]//10
        perm = torch.randperm(self.cur_notes.shape[0])
        self.cur_notes, self.next_notes  = self.cur_notes[perm], self.next_notes[perm]
        self.seeds = self.cur_notes[:N].to(processor)
        self.cur_notes = self.cur_notes[N:].to(processor)
        self.next_notes = self.next_notes[N:].to(processor)
...
def midi_cleaner(data_folder, allowed_time_sigs, allowed_keys,
 max_time_changes=1, max_key_changes=1, ignore_filters=False):
    ...
    --ignore_filters--
    If true, all MIDI files in the folder will be converted regardless of filter
    settings

    --pickle_result--
    If true, the resulting list of pretty_midi objects will be saved as a .pkl file

    --path--
    The path where the .pkl file will be saved
...
sensitivity=0.2, transpose_notes=False, drums=False):
    ...
    --subdiv--
    The resolution at which to sample notes in the song, where subdiv is
    the number of steps per quarter note.

    --sensitivity--
    This is a threshold that allows a window for notes that don't fall exactly
    on the metrical grid to be included. Specified as a percentage of step
    length. Useful for encoding e.g. live performances.
    ...
    piano_roll = torch.zeros((num_measures * subdiv * 4, 128)).to(processor)
...
def roll_to_midi(piano_roll, subdiv=4, program=82,
 tempo=120, resolution=480, pitch_offset=0):
    ...
    --subdiv--
    The number of steps per quarter note. It is important that this has the
    same value as when the array was created in order to make sure note
...
def transpose(mid, semitones):
    ...
    --semitones--
    The number semitones to transpose the notes up (positive)
    or down (negative).
    ##########################################################################
    --------------------------------------------------------------------------
    '''
